...
if nargin==0 % no inputs, just fix the path and run the gui
  ...
  if exist('mmreader')~=2 || exist('VideoReader')~=2
    beep
    disp('DLTdv5 requires mmreader or VideoReader.')
...
%% case 99 - GUI figure creation
  case {99} % Initialize the GUI
    ...
    fprintf('\n')
    disp('DLTdv5 (updated Feb 19, 2013)')
    fprintf('\n')
  ...
  case {0} % Initialize button press from the GUI
    ...
    for i=1:uda.nvid
      ...
      [fname1,pname1] = uigetfile( ...
        {'*.avi;*.cin;*.cine;*.mp4;*.mrf', ...
        'All movie files (*.avi, *.cin, *.cine, *.mp4, *.mrf)'; ...
        '*.avi','AVI movies (*.avi)'; ...
...
%% case 3 - mouse click on image callback
  case {3} % handle button clicks in axes
    ...
    if strcmp(get(gcbo,'Tag'),'VideoFigure')
      ...
      axn=find(h==get(gcbo,'Parent'))-300; % axis number
...
while fr < frmax && trackFailed==false && get(h(35),'Value')~=1
  ...
  for A=1:numel(AAxn)
    ...
    mov=mediaRead(fname,fr+offset,get(h(12),'Value'),get(h(350+A),'Value'));
    imdat=mov.cdata(:,:,:);
  ...
  set(h(5),'Value',fr);
  DLTdv5(1,uda); % call self for video refresh
...
for i=1:uda.nvid
  ...
  if isnan(uda.dltpts(fr,1,sp))==0 && dltvf==1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    plot(h(i+300),xy(1),xy(2),'gd','HitTest','off');
  end
  ...
  if isnan(uda.xypts(fr,i*2,sp)) && isnan(uda.dltpts(fr,1,sp)) ...
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    plot(h(i+300),xpts,ypts,'b-','HitTest','off');
  end
