...
if nargin==0 % no inputs, run the gui initialization routine
  ...
  if exist('mmreader')~=2 && exist('VideoReader')~=2
    beep
    disp('DLTcal5 requires mmreader or VideoReader.')
    disp('Check the Mathworks online documentation to learn about these functions.')
    return
  end
...
switch call
  ...
  case {99} % Initialize the GUI
    ...
    disp(sprintf('\n'))
    disp('DLTcal5 (updated July 9, 2013)')
    disp(sprintf('\n'))
  ...
  case {0} % Initialize button press from the GUI
    ...
    % get the image files
    [calfnames,uda.calpname]=uigetfile( {'*.bmp;*.tif;*.jpg;*.avi;*.cin', ...
    ...
    for i=1:uda.numpts-1
      ptstring(1,i*5-4:i*5)=sprintf('%4d|',i);
    end
    ptstring(1,uda.numpts*5-4:uda.numpts*5-1)=sprintf('%4d',uda.numpts);
  ...
  case {3} % handle button clicks in axes
    ...
    if(get(h(35),'Value')==1)
      ...
      ptval=get(h(32),'Value'); % selected point
      if ptval<ptnum % increase point value if possible
        set(h(32),'Value',ptval+1);
        ptval=ptval+1;
      end
    else
...
if ischar(fpathname)
  ...
  mov=mediaRead(fpathname,1,incolor);
end
...
if ischar(fname)
  if strcmpi(fname(end-3:end),'.avi') || strcmpi(fname(end-3:end),'.mp4')
...
else % fname is not a char so it is an mmreader obj
  mov.cdata=read(fname,frame);
  if incolor==false
    mov.cdata=flipud(mov.cdata(:,:,1));
  else
